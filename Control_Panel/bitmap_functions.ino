...
void drawBMP(char *filename, int x, int y, boolean flip) {
  ...
  if (read16(bmpFile) == 0x4D42) { // BMP file start signature check
    ...
    if ((read16(bmpFile) == 1) && (read16(bmpFile) == 24) && (read32(bmpFile) == 0)) {
      ...
      switch (rotation) {
        case 0:
          if (flip) y = Screen.height() - y - h; break;
        case 1:
          y = Screen.height() - y - h; break;
          break;
        case 2:
          if (flip) y = Screen.height() - y - h; break;
          break;
      ...
      Screen.setWindow(x, y, x + w - 1, y + h - 1);
        ...
        for (col = 0; col < w; col++) { // For each column...
          ...
          if (sd_ptr >= sizeof(sdbuffer)) {
            ...
            if (tft_ptr) {
              ...
              Screen.pushColors(tftbuffer, tft_ptr);
      ...
      if (tft_ptr) Screen.pushColors(tftbuffer, tft_ptr);
